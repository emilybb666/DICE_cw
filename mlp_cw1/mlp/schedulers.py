...
class CosineAnnealingWithWarmRestarts(object):
    ...
    def __init__(self, min_learning_rate, max_learning_rate, total_iters_per_period, max_learning_rate_discount_factor,
                 period_iteration_expansion_factor, current_epochs = 0):
        """
    ...
    def update_learning_rule(self, learning_rule, epoch_number):
        ...
        cur_factor = 0
        while True :
            totalfactor += self.period_iteration_expansion_factor**n
            if (epoch_number < self.total_epochs_per_period * totalfactor) : break
            cur_factor += self.period_iteration_expansion_factor**n
            n += 1

        if (n==0) :
            self.current_epochs = epoch_number
        else :
            self.current_epochs = epoch_number - self.total_epochs_per_period*cur_factor


        learning_rule.learning_rate = self.min_learning_rate + 0.5*(self.max_learning_rate*self.max_learning_rate_discount_factor**n-self.min_learning_rate)*(1+math.cos(math.pi*self.current_epochs/self.total_epochs_per_period*self.period_iteration_expansion_factor**n))
